    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl px-6 py-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            <span className="font-medium text-blue-600">{epc?.plan} Plan</span>
            {' · '}
            Active Districts:{' '}
        ...
        <div className="flex items-center gap-1.5 bg-yellow-50 border border-yellow-200 px-3 py-1.5 rounded-lg">
          ...
          <span className="text-yellow-600 text-sm font-semibold">{epc?.rating?.toFixed(1) || '0.0'}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard
          ...
          icon={<svg className="w-5 h-5 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>}
        />
        <StatCard
          label="New Orders"
        ...
        <StatCard
          ...
          color={planColors[epc?.plan] || 'text-gray-600'}
          bgColor="bg-gray-100"
          onClick={() => navigate('/epc/plan')}
          icon={<svg className="w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>}
        />
      </div>

      {/* Quick actions */}
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <h3 className="text-gray-700 text-sm font-semibold mb-4">Quick Actions</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'View Enquiries', path: '/epc/enquiries', cls: 'bg-blue-50 border-blue-200 text-blue-600 hover:bg-blue-100' },
            { label: 'My Orders',      path: '/epc/orders',    cls: 'bg-green-50 border-green-200 text-green-600 hover:bg-green-100' },
            { label: 'Set Calendar',   path: '/epc/settings',  cls: 'bg-yellow-50 border-yellow-200 text-yellow-600 hover:bg-yellow-100' },
            { label: 'Upgrade Plan',   path: '/epc/plan',      cls: 'bg-purple-50 border-purple-200 text-purple-600 hover:bg-purple-100' },
          ].map((a) => (
